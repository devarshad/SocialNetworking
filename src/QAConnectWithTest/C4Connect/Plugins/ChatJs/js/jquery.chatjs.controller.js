...
var ChatController = (function () {
    function ChatController(options) {
        ...
        this.options.adapter.init(function () {
            ...
            _this.options.adapter.client.onMessagesChanged(function (message) {
                if (!_this.findPmWindowByOtherUserId(message.FromID)) {
                    _this.createPmWindow(message.FromID, true, true);
                }
                //if (message.ToID && message.ToID == this.options.userId && !this.findPmWindowByOtherUserId(message.FromID)) {
                //    this.createPmWindow(message.FromID, true, true);
                //}
            });
            ...
            friendsWindowOptions.Url = _this.options.Url;
            friendsWindowOptions.ProfilePictureUrl = _this.options.ProfilePictureUrl;
    ...
    ChatController.prototype.createPmWindow = function (otherUserId, isMaximized, saveState) {
        ...
        chatPmOptions.Url = this.options.Url;
        chatPmOptions.ProfilePictureUrl = this.options.ProfilePictureUrl;
    ...
    ChatController.prototype.organizePmWindows = function () {
        ...
        var _browserWindowWidth = $(window).width();
        for (var i = 0; i < this.pmWindows.length; i++) {
            var availableSpace = _browserWindowWidth - rightOffset - this.pmWindows[i].pmWindow.getWidth();

            //  var myWidth = this.pmWindows[i].pmWindow.getWidth();
            if (availableSpace < 0) {
                if (i > 0) {
                    this.pmWindows[i - 1].pmWindow.chatWindow.removeWindow();
                    this.pmWindows[i - 1].pmWindow.options.onClose(this.pmWindows[i - 1].pmWindow);
                    break;
                } else {
                    rightOffset -= this.mainWindow.getWidth();
                }
            }
