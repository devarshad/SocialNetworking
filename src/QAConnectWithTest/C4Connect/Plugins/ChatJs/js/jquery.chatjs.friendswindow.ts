...
class ChatFriendsWindow implements IWindow<ChatFriendsWindowState> {
    constructor(options: ChatFriendsWindowOptions) {
        ...
        chatWindowOptions.onCreated = window => {
            ...
            window.$windowTitle.css('width', 182);
            //filter and setting
            var $windowFilter = $("<div/>").addClass("chat-window-filter").appendTo(window.$windowContent);

            var $textBox = $("<input />").addClass("chat-window-filter-text-box").appendTo($windowFilter);

            var $optionBtn = $("<button />").addClass("chat-window-filter-option-btn").appendTo($windowFilter);
        };
