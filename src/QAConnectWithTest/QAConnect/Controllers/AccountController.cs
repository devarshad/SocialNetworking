...
namespace QAConnect.Controllers
{
    ...
    public class AccountController : BaseController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        Logs.Debug("After generating token");

                        var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                        await UserManager.SendEmailAsync(user.Id, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackUrl + "\">here</a>");

                        Logs.Debug("After email sent");
                ...
                {
                    Logs.Error("Error in registration", ex);
                }
            }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                AddAlertMessage("User not found.", DataTypes.Enums.MessageType.Error);
                return View(model);
            }
            ...
            switch (result)
            {
                ...
                default:
                    AddAlertMessage("Invalid login attempt.", DataTypes.Enums.MessageType.Error);
                    return View(model);
            }
        }

        /// <summary>
        /// login to api exapmle
        /// </summary>
        /// <param name="userName"></param>
        /// <param name="password"></param>
        /// <returns></returns>
        public async Task<TokenResponseModel> LoginAPIAsync(string userName, string password)
        {
            using (var client = new HttpClient())
            {
                var result = await client.PostAsync(BuildActionUrl("/Token"), new FormUrlEncodedContent(new List<KeyValuePair<string, string>>
                {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = await UserManager.FindByNameAsync(model.SeachWord);
                if (user == null)
                {
        ...
        {
            return code == null ? View("Error") : View(new { code = code });
        }
