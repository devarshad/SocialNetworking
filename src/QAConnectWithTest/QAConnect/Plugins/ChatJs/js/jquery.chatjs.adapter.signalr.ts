...
interface IChatJsHubProxyClient {
    ...
    updateUserList: (userInfo: ChatUserInfo) => void;
    roomListChanged: (roomListChangedInfo: ChatRoomListChangedInfo) => void
}

interface IChatJsHubProxyServer {
    sendMessage: (message: ChatMessageInfo) => JQueryPromise<any>;
    sendTypingSignal: (roomId: number, conversationId: number, ToID: number) => JQueryPromise<any>;
...
class SignalRServerAdapter implements IServerAdapter {
    ...
    sendMessage(message: ChatMessageInfo, done: () => void): void {
        message.RoomId = 1;
        message.ConversationId = 1;
        this.hubServer.sendMessage(message).done(() => {
            done();
        });
    }

    // sends a typing signal to a room, conversation or user
    sendTypingSignal(roomId: number, conversationId: number, ToID: number, done: () => void): void {
        roomId = 1;
        conversationId = 1;
        this.hubServer.sendTypingSignal(roomId, conversationId, ToID).done(() => {
            done();
        });
    }
...
class SignalRClientAdapter implements IClientAdapter {
    ...
    constructor(chatHubClient: IChatJsHubProxyClient) {
        ...
        this.hubClient.updateUserList = (userInfo: ChatUserInfo) => {
            this.triggerUpdateUserList(userInfo);
        };
    ...
    // adds a handler to the onUserConnected event
    onUpdateUserList(handler: (userInfo: ChatUserInfo) => void): void {
        this.updateUserListHandlers.push(handler);
    }
    ...
    triggerUpdateUserList(userInfo: ChatUserInfo): void {
        for (var i = 0; i < this.updateUserListHandlers.length; i++)
            this.updateUserListHandlers[i](userInfo);
    }
    ...
    updateUserListHandlers: Array<(userInfo: ChatUserInfo) => void>;
